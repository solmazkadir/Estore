...
namespace Estore.WebAPIUsing.Utils
{
    public class MailHelper
    {
        public static async Task SendMailAsync(Contact contact, string konu = "Siteden mesaj geldi")
        {
            ...
            message.From = new MailAddress("info@siteadi.com"); //mesaj gönderildiği adres
            message.To.Add("info@siteadi.com"); //mesajın gönderileceği mail adresi
            message.To.Add("test@siteadi.com"); //1den fazla yere mail gönderebiliriz
            ...
            //smtpClient.Send(message); //mesajı senkron olarak gönder
           await smtpClient.SendMailAsync(message); //mesajı asenkron gönder
